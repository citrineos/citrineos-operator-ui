...
// Add a new query to fetch by business ID (id + tenantId)
export const CHARGING_STATIONS_GET_BY_BUSINESS_ID_QUERY = gql`
  query GetChargingStationByBusinessId($id: String!, $tenantId: String!) {
    ChargingStations(
      where: { id: { _eq: $id }, tenantId: { _eq: $tenantId } }
      limit: 1
    ) {
      pkId
      tenantId
...
export const CHARGING_STATIONS_EDIT_MUTATION_DEPRECATED = gql`
  mutation ChargingStationsEditDeprecated(
    $id: String!
    $tenantId: String!
    $object: ChargingStations_set_input!
  ) {
    update_ChargingStations(
      where: { id: { _eq: $id }, tenantId: { _eq: $tenantId } }
      _set: $object
    ) {
      returning {
...
export const CHARGING_STATIONS_DELETE_MUTATION_DEPRECATED = gql`
  mutation ChargingStationsDeleteDeprecated($id: String!, $tenantId: String!) {
    delete_ChargingStations(
      where: { id: { _eq: $id }, tenantId: { _eq: $tenantId } }
    ) {
      returning {
        pkId
